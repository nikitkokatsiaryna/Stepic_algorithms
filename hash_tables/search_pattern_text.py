# # variant 1. Failed in the 6 test
class PatternText:
    def __init__(self, pattern):
        self.pattern = pattern
        self._size = len(pattern)
        self._p = 1_000_000_007
        self._x = 263

    def get_mult(self, index):
        x_mult = 1

        for el in range(index):
            x_pow = self._x % self._p
            x_mult *= x_pow

        return x_mult % self._p
    ...
    def get_hash(self, value):
        ...
        for index, char in enumerate(value):
            a = ((ord(char) % self._p) * self.get_mult(index)) % self._p
            total += a
        hash_char = (total % self._p) % m

        return hash_char


def main():
    table, text = PatternText(input()), input()
    pattern = table.pattern
    pattern_len = len(pattern)
    hash_pattern = table.get_hash(table.pattern)
    index_list = set()

    for el in range(len(text)):
        part_text = text[el:el + pattern_len]
        hash_part_text = table.get_hash(part_text)
